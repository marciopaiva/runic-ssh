...
| `ChromeProposalOrbitHostKey.dc.html` | Exploratory, the same idea's answer to "does everything go through the pill": no. `build_hostkey()`'s copy and fields reused verbatim in a summoned card over the veiled scene, and the bottom pill itself swaps to a dimmed, inert "Locked until this is resolved" rather than staying open, per CLAUDE.md section 7 rule 3. Declined along with `ChromeProposalOrbit.dc.html`; `HostKey.dc.html` is still the shipped prompt |
| `ChromeProposalOrbitWorkspaces.dc.html` | Exploratory, the other half of that same answer: `home_rail()`'s five slots (Home/Monitor/Sessions/SFTP/Map) as a small icon cluster docked at the frame's top-left corner instead of a 48px rail column, present the same way the window controls are rather than summoned. Declined along with `ChromeProposalOrbit.dc.html`; `Main.dc.html`'s rail is still shipped |
...
| `MonitorLogs.dc.html` | Monitor, a path tailed the same way, typed or picked from files `find` actually located under `/var/log` on this host, for a service that logs to a plain file instead of (or as well as) the journal |
