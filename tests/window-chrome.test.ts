...
describe('session tabs', () => {
  it('gives a tab to an open connection', () => {
    expect(
      openTabs([live('a', 'connected', 7)], new Set(), new Set(['a'])).map((t) => t.sessionId),
    ).toEqual(['a']);
  });
  ...
  it('gives a tab to one still connecting', () => {
    ...
    expect(
      openTabs([live('a', 'connecting')], new Set(), new Set(['a'])).map((t) => t.sessionId),
    ).toEqual(['a']);
  });
  ...
  it('gives no tab to a host nobody has connected to', () => {
    ...
    expect(openTabs([live('a', 'saved'), live('b', 'unreachable')], new Set(), new Set(['a', 'b']))).toEqual([]);
  });
...
describe('a connection Sessions never asked for (ADR-0053)', () => {
  ...
  it('gives no tab to a connection open only because SFTP is using it', () => {
    expect(openTabs([live('a', 'connected', 7)], new Set(), new Set())).toEqual([]);
  });

  it('gives no tab to an SFTP-initiated attempt stopped on a host key either', () => {
    /* `attentionIds` used to bypass every other check. The decision itself
       renders over the SFTP pane, not a Sessions tab nobody asked for. */
    expect(openTabs([live('a', 'keyMismatch')], new Set(['a']), new Set())).toEqual([]);
  });

  it('still gives a Sessions-initiated attempt its tab while unresolved', () => {
    expect(
      openTabs([live('a', 'keyMismatch')], new Set(['a']), new Set(['a'])).map((t) => t.sessionId),
    ).toEqual(['a']);
  });
...
describe('the tab a question is waiting in', () => {
  ...
  it('keeps the tab of a session whose attempt failed', () => {
    expect(
      openTabs([live('a', 'unreachable')], new Set(['a']), new Set(['a'])).map((t) => t.sessionId),
    ).toEqual(['a']);
  });
  ...
  it('drops the tab once the attempt is let go', () => {
    ...
    expect(openTabs([live('a', 'unreachable')], new Set(), new Set(['a']))).toEqual([]);
  });
  ...
  it('does not give a second tab to a session that already has one', () => {
    ...
    const tabs = openTabs([live('a', 'connected', 3)], new Set(['a']), new Set(['a']));
  ...
  it('reports the state the session is actually in', () => {
    ...
    expect(openTabs([live('a', 'unreachable')], new Set(['a']), new Set(['a']))[0]?.kind).toBe('unreachable');
  });
});
